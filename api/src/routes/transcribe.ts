...
export async function transcribeRoute(fastify: FastifyInstance) {
  fastify.post("/transcribe", async (request, reply) => {
    ...
    fastify.log.info(`Language received: ${language}`);
    fastify.log.info(`Fields: ${JSON.stringify(fields)}`);
    ...
    try {
      ...
      const { text: rawText, detectedLanguage } =
        await whisperService.transcribe(audioBuffer, language);

      fastify.log.info(`Whisper detected language: "${detectedLanguage}"`);

      fastify.log.info("Streaming clean text with LLaMA...");
      const stream = await cleanerService.cleanStream(
        rawText,
        detectedLanguage,
      );
      ...
      reply.raw.write(
        `data: ${JSON.stringify({ error: "Transcription failed" })}\n\n`,
      );
      reply.raw.end();
    }
  });
